...
The main text we follow is [@Gelman_Hill_Vehtari_2020], available for [free download](https://avehtari.github.io/ROS-Examples/index.html)
...
```{r}
#| message: false
#| code-summary: "Code 3.4 Comparison of simple linear regression with classical and Bayesian inference"
...
p1 <- ggplot(lion_noses, aes(percentage.black, age)) +
    ...
    ylab("age (years)")
p2 <- ggplot(lion_noses, aes(percentage.black, age)) +
    geom_point() +
    geom_abline(intercept = 0.9, slope = 0.1) +
    theme_bw() +
    xlab("percentage black on nose") +
...
```{r}
#| message: false
#| code-summary: "Code 3.5 Plot uncertainty"
lion_noses %>%
    tidybayes::add_epred_draws(m2) %>%
...
final section shiny app?? Check Morega
