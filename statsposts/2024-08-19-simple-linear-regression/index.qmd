...
The main text followed is [@Gelman_Hill_Vehtari_2020], available for [free download](https://avehtari.github.io/ROS-Examples/index.html). Supplementary recommended books are [@Fox_Weisberg_2018].
...
ggplot(lion_noses, aes(percentage.black, age)) +          # <2>
  ...
  ylab("Age (years)") +                                   # <5>
    ...
    annotate("text", x = 22, y = 0, label = "91% of lions under 5 years have less than 40% nose black", colour = "blue", size = 3) +                 # <6>
...
When the model m1 is run, a large amount of information is saved into m1, now visible in the R Studio environment. The essentials for interpreting the model are shown above, with the `print(m1)` command, but we could also look at any of the 27 stored objects individually, most of which we will not use, for example the coefficients with `m1$coefficients`, which gives the same information as the Median column in the summary, although to more decimal places. This option to look more carefully at the output, rather than be presented with just a summary, is distinctive to R and other coding softwares.

The output is telling us, for now ignoring statistical uncertainty, that the best fit to the proposed model `y = intercept + slope*percentage.black + error` is:
...
```{r}
#| code-summary: "Code 3.6 A comparison of classical and Bayesian models"
#| code-block-bg: true
#| output: false
#| highlight-style: github
models <- list(
...
```{r}
#| code-summary: "Code 3.7 A comparison of classical and Bayesian models"
#| code-block-bg: true
#| highlight-style: github
#| warning: false
...
```{r}
#| message: false
#| code-summary: "Code 3.9 A plot with CIs"
#| code-block-bg: true
#| highlight-style: github
...
```{r}
#| message: false
#| code-summary: "Code 3.7 Plot uncertainty"
lion_noses %>%
    tidybayes::add_epred_draws(m1) %>%
    ggplot(aes(x = percentage.black, y = age)) +
    stat_lineribbon(aes(y = .epred), .width = c(0.5, 0.95)) +
...
added quarto [line-highlight](https://github.com/shafayetShafee/line-highlight#line-highlight-extension-for-quarto) extension for highlighting code, including output code
